function [ua_iter, tk_xj,cg_it] = NLFIE_HAAR_S2(fx,dkut, n, iter,tol)
...
% BdJx{1}=pinv(P_ns_new-dkut_xt*P_ns_new(2:n-1,:)./length(tk_xj));
%% 主函数
for i=2:iter+1  
    ...
    % /((delta_a')*delta_gx);% Sherman–Morrison formula
...
% temp=ones(length(x)-2);
% temp2=[temp;ones(1,length(u));ones(1,length(u))];
% kt=sin(pi.*t').* (u.^3);
% kf=temp2*kt.*1/5.*cos(pi.*x')./(length(x)-2);
% t_new=t.*u;
%     [X, T] = ndgrid(x, t_new);
