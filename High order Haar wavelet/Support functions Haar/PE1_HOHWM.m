function [output] = PE1_HOHWM(RF,input_coefficient,n)
% 用来描述函数误差
% RF表示理论函数, 输入类型为函数句柄
...
j=ceil(log(length(input_coefficient))/log(2));s=1;
[alpha,beta,gamma]=generate_haar(j);
% nn=CP(n);
nn=linspace(0,1,n);
P_ns=zeros(length(input_coefficient)-1,n);
for i=1:length(input_coefficient)-1
