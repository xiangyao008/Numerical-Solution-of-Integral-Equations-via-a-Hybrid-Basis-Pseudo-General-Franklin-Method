%% Haar Wavelet Method for NLFIE (S1 Version)
% 包含循环测速与基于全局范数的收敛阶分析
...
% --- 问题定义 (Example) ---
fx = @(x) sin(pi*x);
% 注意：虽然定义了 kut 和 dkut，但您的函数调用仅使用了 fx
% 若 NLFIE_HAAR_S1 需要这些参数，请在下方函数调用中补全
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 Haar NLFIE 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up
fprintf('正在进行预热运行...\n');
[~, ~, ~] = NLFIE_HAAR_S1(fx, n, iter, tol);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj1, cg_it1] = NLFIE_HAAR_S1(fx, n, iter, tol);
...
[ua_iter2, tk_xj2, cg_it2] = NLFIE_HAAR_S1(fx, n/2, iter, tol);

% 注意：原代码此处可能有笔误 (使用了 cg_it1)，这里修正为 cg_it2
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('Haar NLFIE Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
