...
%% 1. 问题定义与参数设置
n = 256;            % 细网格阶数 (Fine Grid)
iter = 100;         % 最大迭代次数
tol = 1e-6;         % 迭代容差
vp = 1024;          % 验证点数量 (提升至 1024 以获得更精确的统计)
num_runs = 10;      % 测速循环次数
...
% Volterra 核函数 (虽然求解器可能只用了导数，但保留定义以供参考)
kf = @(x,t,u) (x-t).*(u.^2);
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 Haar S2 NLFVIE 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up
fprintf('正在进行预热运行...\n');
[~, ~, ~] = NLFVIE_HAAR_S2_Simpson(fx, dvkut, dfkut, n, iter, tol);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj1, cg_it1] = NLFVIE_HAAR_S2_Simpson(fx, dvkut, dfkut, n, iter, tol);
...
[ua_iter2, tk_xj2, cg_it2] = NLFVIE_HAAR_S2_Simpson(fx, dvkut, dfkut, n/2, iter, tol);
...
%% 4. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('Haar S2 NLFVIE Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
