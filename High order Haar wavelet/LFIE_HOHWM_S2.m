...
% 参数命名定义形式参考这篇文章
%%
%% Higher order Haar wavelet method (S2 Version) for numerical solution of integral equations
%%
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 Haar S2 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up
fprintf('正在进行预热运行...\n');
[~, ~] = LFIE_HAAR_S2(fx, kf, n);
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('Haar Wavelet (S2) 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
