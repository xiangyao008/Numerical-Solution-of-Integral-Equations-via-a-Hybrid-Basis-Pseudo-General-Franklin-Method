%% Higher order Haar wavelet method (S2 Version) for LVIE
% 参数命名定义形式参考这篇文章
%%
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 Haar S2 LVIE 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up
fprintf('正在进行预热运行...\n');
[~, ~] = LVIE_HAAR_S2(fx, kf, n);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [Array_fx1, tk_xj1] = LVIE_HAAR_S2(fx, kf, n);
    
    % 计算细网格误差
    % 变量名重命名为 _fine 以明确含义
    pointwise_error_fine = PE2_HOHWM(RF, Array_fx1, vp);
...
[Array_fx2, tk_xj2] = LVIE_HAAR_S2(fx, kf, n/2);
pointwise_error_coarse = PE2_HOHWM(RF, Array_fx2, vp);

% --- 全局误差范数与收敛阶计算 (去除首尾边界点) ---

% 1. 数据切片：去除第1个和最后1个点 (Interior Points Only)
% Volterra 方程起始点误差常为0，剔除以避免 log(0)
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('Haar S2 LVIE Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
