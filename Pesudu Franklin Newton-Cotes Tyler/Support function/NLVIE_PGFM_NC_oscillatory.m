function [ua_iter, tk_xj, cg_it] = NLVIE_PGFM_NC_oscillatory(fx, dkut, n, iter, tol, K, kf_f, init)
% NLVIE_PGFM_NC: Nonlinear Volterra Integral Equation Solver
%
% 参照 NLFIE_PGFM_NC 风格重构，保持了变量命名和代码结构的一致性。
% 针对 Volterra 方程引入了高效的区间掩码（Masking）积分算法。

%% 1. 介绍和思路
% 伪Franklin方法中的dkut_xt的维度以及迭代过程中所使用的矩阵维度选择
% dkut_xt=(n+K+1)*(n), Pg_ns1=(n)*(n+K+1) -> Pg_ns1'=(n+K+1)*(n)

%% 2. 逆Broyden秩1方法初始化
% cp=collocation points
pseudo_Franklin_basis = @(x, cp, K) 0.*((0<=x) < cp) + ((x - cp).^K).*(cp<=x & x <= 1);

Pg_ns1 = zeros(n+K+1, n);
Pg_ns2 = zeros(n+K+1, n+K+1);

% 生成配置点和扩展节点
tk_xj = CPPF(n); 
% 强制转换为列向量，确保维度一致性
tk_xj = tk_xj(:);

tk_xj_new = [linspace(0, tk_xj(1), ceil(K/2)+1), tk_xj(2:n-1)', linspace(tk_xj(n), 1, ceil((K+1)/2)+1)]';
tk_xj_new = tk_xj_new(:);

% 构造基函数矩阵 (向量化优化版)
for i = 0:K
    Pg_ns1(i+1, :) = tk_xj.^(i); 
    Pg_ns2(i+1, :) = tk_xj_new.^(i); 
end

if n > 0
    % Franklin Basis
    centers = tk_xj(1:n);
    centers = centers(:)'; % 确保为行向量 (1 x n)
    
    % Pg_ns1 (Source)
    X1 = tk_xj; % 列向量 (n x 1)
    D1 = bsxfun(@minus, X1, centers); % (n x n)
    Pg_ns1(K+2:end, :) = (D1.^K)' .* ((D1 > 0 & X1 <= 1)');
    
    % Pg_ns2 (Target)
    X2 = tk_xj_new; % 列向量 (M x 1)
    D2 = bsxfun(@minus, X2, centers); % (M x n)
    Pg_ns2(K+2:end, :) = (D2.^K)' .* ((D2 > 0 & X2 <= 1)');
end

Pg_ns1 = Pg_ns1';
Pg_ns2 = Pg_ns2';

%% 3. 预计算 Volterra 积分缓存 (关键效率优化)
% 为了与 NLFIE 结构保持一致，这里将积分所需的静态数据打包进 Cache
% 避免在主循环的 get_kf_nc 中重复计算网格
Cache = init_volterra_cache(tk_xj_new, n, K);

%% 3. Jacobian 矩阵初始化 (深度优化版)
Init_ux = zeros(n+K+1, 1); 
Init_ux(1:K+1) = init;

ua_iter = zeros(n+K+1, iter+1);
ua_iter(:, 1) = Init_ux;

u1 = Pg_ns1 * ua_iter(:, 1); % 获得关于t的初值函数矩阵 (n x 1)

% --- 优化 1: 向量化计算初始 Jacobian (dkut_xt) ---
% 避免双重循环，利用矩阵广播一次性计算所有元素
% 构造网格: X (Target points), T (Source points)
[T_mat, X_mat] = meshgrid(tk_xj, tk_xj_new); 

% 构造 U 矩阵: u1 对应 T_mat 的列变化 (Source u)
% u1 是 (n x 1)，我们需要将其转置并复制以匹配 (M x n)
U_mat = repmat(u1(:)', length(tk_xj_new), 1);
Volterra_Mask = (T_mat <= X_mat + 1e-12);
try
    J_raw = dkut(X_mat, T_mat, U_mat);
    dkut_xt = J_raw .* Volterra_Mask;
catch
    % 如果 dkut 不支持向量化，回退到循环但保持对角线优化
    dkut_xt = zeros(length(tk_xj_new), length(tk_xj));
    for i = 1:length(tk_xj_new)
        % 仅循环计算 t <= x 的部分
        valid_idx = tk_xj <= tk_xj_new(i) + 1e-12;
        if any(valid_idx)
            dkut_xt(i, valid_idx) = dkut(tk_xj_new(i), tk_xj(valid_idx), u1(valid_idx));
        end
    end
end

% --- 优化 2: 高效求逆 ---
% 初始 Broyden 矩阵 BdJx 是 (I - K)' 的逆近似
% 计算 Jacobian 近似矩阵 J_approx
J_approx = Pg_ns2 - dkut_xt * Pg_ns1 ./ length(u1);

BdJx = cell(1, iter+1);

BdJx{1} = pinv(J_approx);

% 预先计算 f(x) 以提高效率
fx_val = fx(tk_xj_new);
if isrow(fx_val), fx_val = fx_val'; end

%% 5. 主循环函数
cg_it = iter; % 默认收敛步数

for i = 2:iter+1
    % 1. 获取上一以步的系数和 u(x)
    coeffs_prev = ua_iter(:, i-1);
    
    % 2. 计算当前残差 R(x)
    % 积分项 I(u)
    kf_val = get_kf_nc(tk_xj_new, coeffs_prev, kf_f, Cache);
    % 构造当前 u(x)
    u_curr_vals = (Pg_ns2 * coeffs_prev); 
    % R = u - f - I(u)
    g_x = u_curr_vals - fx_val - kf_val;
    
    % 检查残差范数 (作为收敛判据之一)
    res_norm = norm(g_x);
    if res_norm < tol
        disp(['Converged (Residual) in ', num2str(i-1), ' iterations.']);
        cg_it = i-1;
        break;
    end
    
    % 3. 构建精确 Jacobian 矩阵 J
    % J = Identity_Part - Integral_Part
    % Identity_Part 就是基函数矩阵 Pg_ns2
    % Integral_Part 需要对每一列基函数进行加权积分: int( dK/du * phi_j )
    
    % 3.1 计算导数核函数 dK/du 在当前 u 下的值
    % 利用 Cache 计算 dK_val，结构与 kf 类似
    dK_val = get_dk_nc(tk_xj_new, coeffs_prev, dkut, Cache);
    
    % 3.2 组装 Jacobian (耗时步骤，但为了精确度是值得的)
    % 积分项矩阵 Integral_Mat 的第 j 列 = int( dK(x,t,u) * phi_j(t) )
    Integral_Mat = compute_jacobian_integral(dK_val, Cache);
    
    % J = Phi(x) - Int( dK/du * Phi(t) )
    J = Pg_ns2 - Integral_Mat;
    
    % 4. 求解线性方程组 J * delta = -g_x
    % 使用 MATLAB 左除，自带数值稳定性处理
    delta_a = J \ (-g_x);
    
    % 5. 更新系数
    % 加入阻尼因子 (Damping) 防止初期步长过大
    step_size = 1.0; 
    ua_iter(:, i) = coeffs_prev + step_size * delta_a;
    
    % 6. 收敛性检查 (基于步长)
    update_norm = norm(delta_a);
    current_norm = norm(coeffs_prev);
    if update_norm / (current_norm + 1e-14) < tol
        disp(['Converged (Step Size) in ', num2str(i-1), ' iterations.']);
        cg_it = i-1;
        break;
    end
end

% 截断未使用的迭代空间
ua_iter = ua_iter(:, 1:cg_it);

end

%% 辅助函数：Volterra 积分计算 (核心优化)
function kf = get_kf_nc(x_nodes, coeffs, kernel_func, Cache)
    % NLFIE 风格的积分接口，但在内部利用 Cache 进行 Volterra 快速计算
    
    % 1. 快速评估 u(t) 在所有积分点的值
    U_flat = Cache.Phi_Integration * coeffs;
    
    % 2. Reshape 为 3D 矩阵 [1, SubPoints, Intervals]
    U_3D = reshape(U_flat, 1, Cache.num_sub_points, Cache.num_intervals);
    
    % --- 维度修复开始 ---
    % 获取各维度大小
    [num_tar, ~, ~] = size(Cache.X_3D); % [N, 1, 1]
    [~, num_sub, num_int] = size(Cache.T_3D); % [1, M, K]
    
    % 显式扩展所有变量到全网格 [N, M, K]
    % 这样无论 kernel_func 内部如何实现 (即使 x 被视为常数或忽略)，
    % 输入和输出的维度都将严格保持为 [N, M, K]。
    X_Full = repmat(Cache.X_3D, [1, num_sub, num_int]);
    T_Full = repmat(Cache.T_3D, [num_tar, 1, 1]);
    U_Full = repmat(U_3D, [num_tar, 1, 1]);
    
    % 3. 计算核函数 K(x, t, u)
    Kernel_Val = kernel_func(X_Full, T_Full, U_Full);
    % --- 维度修复结束 ---
    
    % 4. 积分加权求和 (Newton-Cotes Weights)
    % Cache.W_3D 维度为 [1, M, 1]，利用广播计算
    Segment_Integrals = sum(Kernel_Val .* Cache.W_3D, 2); 
    
    % 5. 应用区间长度 Jacobian 和 Volterra 掩码
    % 仅累加完全在 x 左侧的区间 (Interval_Mask)
    % Sum over intervals (Dim 3)
    kf = sum(Segment_Integrals .* Cache.H_3D .* Cache.Interval_Mask, 3);
    
    if isrow(kf), kf = kf'; end
end

%% 辅助函数：缓存初始化
function Cache = init_volterra_cache(x_nodes, n, K)
    % 确定积分阶数 m
    if K <= 1, m = 1; elseif K <= 2, m = 2; elseif K == 3, m = 3; else, m = 4; end       
    [nc_nodes, nc_weights] = get_nc_weights(m);
    
    x_targets = x_nodes(:); 
    num_sub = length(nc_nodes);
    num_tar = length(x_targets);
    
    % 构建全局积分网格 (必须包含0和所有x_nodes)
    unique_nodes = unique([0; x_targets]);
    sorted_grid = sort(unique_nodes);
    
    h_vec = diff(sorted_grid);
    num_int = length(h_vec);
    
    interval_starts = sorted_grid(1:end-1);
    interval_ends   = sorted_grid(2:end);
    
    % 生成所有积分点 T
    T_matrix = nc_nodes * h_vec' + interval_starts'; 
    T_flat = T_matrix(:); 
    
    % 预计算基函数矩阵 Phi (复用 evaluate_basis 逻辑)
    Phi = evaluate_basis_matrix(T_flat, n, K);
    
    % 打包缓存数据
    Cache.num_sub_points = num_sub;
    Cache.num_intervals  = num_int;
    Cache.Phi_Integration = Phi;
    
    % 预 Reshape 矩阵以便广播
    Cache.T_3D = reshape(T_matrix, 1, num_sub, num_int);
    Cache.X_3D = reshape(x_targets, num_tar, 1, 1);
    Cache.W_3D = reshape(nc_weights, 1, num_sub, 1);
    Cache.H_3D = reshape(h_vec, 1, 1, num_int);
    
    % 预计算 Volterra Mask (区间右端点 <= 目标点x)
    Interval_Ends_3D = reshape(interval_ends, 1, 1, num_int);
    Cache.Interval_Mask = (Interval_Ends_3D <= Cache.X_3D + 1e-12);
end

%% 辅助函数：基函数矩阵生成 (bsxfun 修复版)
function Phi = evaluate_basis_matrix(t, n, K)
    t = t(:); % 强制列向量
    num_coeffs = n + K + 1;
    Phi = zeros(length(t), num_coeffs);
    
    % 多项式部分
    for p = 0:K
        Phi(:, p+1) = t.^p; 
    end
    
    % Franklin 部分
    if n > 0
        tk_map = CPPF(n); 
        centers = tk_map(1 : n);
        centers = centers(:)'; % 强制行向量
        
        % D = (M x 1) - (1 x N) = (M x N)
        D = bsxfun(@minus, t, centers);
        Mask = (D > 0) & (t <= 1); 
        Phi(:, K+2:end) = (D.^K) .* Mask;
    end
end

function [nodes, weights] = get_nc_weights(m)
    nodes = linspace(0, 1, m + 1)';
    switch m
        case 1, weights = [1; 1] / 2;
        case 2, weights = [1; 4; 1] / 6;
        case 3, weights = [1; 3; 3; 1] / 8;
        case 4, weights = [7; 32; 12; 32; 7] / 90;
    end
end

%% 辅助函数：获取 dK/du 的值 (复用积分网格)
function dK_val_3D = get_dk_nc(x_nodes, coeffs, dkut_func, Cache)
    % 计算 dK/du(x, t, u(t)) 的值，返回 3D 矩阵以便后续与基函数相乘
    
    % 1. 恢复 u(t)
    U_flat = Cache.Phi_Integration * coeffs;
    U_3D = reshape(U_flat, 1, Cache.num_sub_points, Cache.num_intervals);
    
    % --- 维度修复开始 ---
    [num_tar, ~, ~] = size(Cache.X_3D);
    [~, num_sub, num_int] = size(Cache.T_3D);
    
    % 显式扩展
    X_Full = repmat(Cache.X_3D, [1, num_sub, num_int]);
    T_Full = repmat(Cache.T_3D, [num_tar, 1, 1]);
    U_Full = repmat(U_3D, [num_tar, 1, 1]);
    
    % 2. 计算导数核
    % 返回维度严格为 (Targets, Sub, Int)
    dK_val_3D = dkut_func(X_Full, T_Full, U_Full);
    % --- 维度修复结束 ---
end

%% 辅助函数：计算 Jacobian 的积分部分
function J_Int = compute_jacobian_integral(dK_3D, Cache)
    % 目标: 计算矩阵 M，其中 M(i, j) = int_0^{x_i} dK/du * phi_j(t) dt
    
    num_targets = size(dK_3D, 1);
    num_basis = size(Cache.Phi_Integration, 2); % n + K + 1
    J_Int = zeros(num_targets, num_basis);
    
    % 为了效率，我们需要避免对 num_basis 进行显式循环，
    % 但由于 Phi 是 (TotalPoints x Basis)，我们需要将其 Reshape 匹配积分结构
    
    % Cache.Phi_Integration 是 (Total_Integration_Points x Basis)
    % 我们将其重塑为 (1, Sub, Int, Basis) 以便广播
    Phi_4D = reshape(Cache.Phi_Integration, 1, Cache.num_sub_points, Cache.num_intervals, num_basis);
    
    % dK_3D 是 (Targets, Sub, Int) -> 扩展为 (Targets, Sub, Int, 1)
    dK_4D = reshape(dK_3D, num_targets, Cache.num_sub_points, Cache.num_intervals, 1);
    
    % 权重扩展: (1, Sub, 1, 1)
    W_4D = reshape(Cache.W_3D, 1, Cache.num_sub_points, 1, 1);
    
    % 区间长度扩展: (1, 1, Int, 1)
    H_4D = reshape(Cache.H_3D, 1, 1, Cache.num_intervals, 1);
    
    % 掩码扩展: (Targets, 1, Int, 1)
    Mask_4D = reshape(Cache.Interval_Mask, num_targets, 1, Cache.num_intervals, 1);
    
    % --- 核心积分计算 ---
    % Integrand = dK * Phi * Weights * h * Mask
    % 维度: (Targets, Sub, Int, Basis)
    % 注意：这可能会消耗大量内存。如果内存溢出，需要回退到对 Basis 的循环。
    
    try
        % 尝试全向量化计算
        Integrand = dK_4D .* Phi_4D .* W_4D .* H_4D .* Mask_4D;
        % 对 Sub (dim 2) 和 Int (dim 3) 求和
        J_Int = squeeze(sum(sum(Integrand, 2), 3));
        
    catch
        % 内存不足时的回退方案 (按基函数循环)
        % 预计算公共项: dK * W * H * Mask
        % (Targets, Sub, Int)
        Common_Term = dK_3D .* Cache.W_3D .* Cache.H_3D .* Cache.Interval_Mask;
        
        for j = 1:num_basis
            % 取出第 j 个基函数的值 (Total_Points x 1)
            phi_j_flat = Cache.Phi_Integration(:, j);
            phi_j_3D = reshape(phi_j_flat, 1, Cache.num_sub_points, Cache.num_intervals);
            
            % 积分
            Term = Common_Term .* phi_j_3D;
            J_Int(:, j) = sum(sum(Term, 2), 3);
        end
    end
end