...
fprintf('-------------------------------------------\n');
fprintf('正在准备 NLFVIE_GKFM_K2_Simpson 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up (避免首次运行 JIT 编译时间影响)
fprintf('正在进行预热运行...\n');
[~, ~, ~] = NLFVIE_K2_Simpson(fx, dvkut, dfkut, n, iter, tol);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj1, cg_it1] = NLFVIE_K2_Simpson(fx, dvkut, dfkut, n, iter, tol);
...
[ua_iter2, tk_xj2, cg_it2] = NLFVIE_K2_Simpson(fx, dvkut, dfkut, n/2, iter, tol);
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('NLFVIE_GKFM_K2_Simpson Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
