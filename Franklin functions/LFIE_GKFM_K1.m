%% Numerical solution of integral equations using GKFM method (Performance Test Framework)
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 GKFM 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj] = LFIE_K1(fx, kf, n);
    
    % 计算细网格误差
    % 注意：调用的是原始代码中的 PE1 函数
    pointwise_error_fine = PE1(RF, ua_iter1, vp);
...
[ua_iter2, tk_xj2] = LFIE_K1(fx, kf, n/2);
pointwise_error_coarse = PE1(RF, ua_iter2, vp);

% --- 全局误差范数与收敛阶计算 (去除首尾边界点) ---

% 1. 数据切片：去除第1个和最后1个点 (Interior Points Only)
% 此操作旨在避免因局部边界强约束或截断效应导致的奇点干扰整体收敛性评估
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('GKFM Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
