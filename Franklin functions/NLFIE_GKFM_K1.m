...
fprintf('-------------------------------------------\n');
fprintf('正在准备 NLFIE_GKFM_K1 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up (避免首次运行 JIT 编译时间影响)
fprintf('正在进行预热运行...\n');
[~, ~, ~] = NLFIE_K1(fx, n, iter, tol);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj1, cg_it1] = NLFIE_K1(fx, n, iter, tol);
    
    % 计算细网格误差
    % 提取最终收敛步 cg_it1 的结果进行评估
...
[ua_iter2, tk_xj2, cg_it2] = NLFIE_K1(fx, n/2, iter, tol);
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('NLFIE_GKFM_K1 Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
