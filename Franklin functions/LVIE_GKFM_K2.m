%% Numerical solution of Volterra integral equations using GKFM method (Performance Test Framework)
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 LVIE_GKFM_K2 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj] = LVIE_K2(fx, kf, n);
    
    % 计算细网格误差
    % 注意：调用对应 K2 算例的误差评估函数 PE2
    pointwise_error_fine = PE2(RF, ua_iter1, vp);
...
[ua_iter2, tk_xj2] = LVIE_K2(fx, kf, n/2);
pointwise_error_coarse = PE2(RF, ua_iter2, vp);
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('LVIE_GKFM_K2 Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
