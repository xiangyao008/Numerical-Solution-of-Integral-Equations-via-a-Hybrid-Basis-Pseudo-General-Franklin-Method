%% Numerical solution of nonlinear integral equations using GKFM method (Performance Test Framework)
clc; close all; clear all;

% --- 参数定义 ---
n = 16;             % 细网格阶数 (Fine Grid)
iter = 100;         % 最大迭代次数
tol = 1e-6;         % 误差容限
...
fprintf('-------------------------------------------\n');
fprintf('正在准备 NLFIE_GKFM_K2 测试 (Fine n=%d, Coarse n=%d)...\n', n, n/2);

time_records = zeros(num_runs, 1);

% [预热] Warm-up (避免首次运行 JIT 编译时间影响)
fprintf('正在进行预热运行...\n');
[~, ~, ~] = NLFIE_K2(fx, n, iter, tol);
...
for i = 1:num_runs
    ...
    % 求解细网格 (Fine Grid)
    [ua_iter1, tk_xj1, cg_it1] = NLFIE_K2(fx, n, iter, tol);
    
    % 计算细网格误差
    % 提取最终收敛步 cg_it1 的结果进行评估，调用 PE2 函数
    pointwise_error_fine = PE2(RF, ua_iter1(:, cg_it1), vp);
...
[ua_iter2, tk_xj2, cg_it2] = NLFIE_K2(fx, n/2, iter, tol);
% 修正说明：使用 cg_it2 索引，确保提取的是粗网格最终收敛步的结果
pointwise_error_coarse = PE2(RF, ua_iter2(:, cg_it2), vp);
...
%% 3. 结果格式化输出
fprintf('\n-------------------------------------------\n');
fprintf('NLFIE_GKFM_K2 Method 精度与收敛阶分析报告\n');
fprintf('-------------------------------------------\n');
