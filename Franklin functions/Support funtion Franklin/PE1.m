function [output] = PE1(RF,input_coefficient,n)
% 用来描述函数误差
% RF表示理论函数, 输入类型为函数句柄
...
% nn=CP(n);
nn=linspace(0,1,n);
hi_x=zeros(n,length(input_coefficient)-1);
...
% end
for i=1:n
    for j=1:length(input_coefficient)-1
        hi_x(i,j)=NEW_FRANKLIN_function{j+1}(nn(i));
    end
end
temp=ones(n,1);
hi_x_new=[temp,hi_x];
