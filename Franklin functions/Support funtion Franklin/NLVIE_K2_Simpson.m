function [ua_iter,tk_xj,cg_it] = NLVIE_K2_Simpson(fx, dkut, n, iter,tol)
...
for i=1:n-1
    for j=1:(n-2)
        hi_x2(i,j)=NEW_FRANKLIN_function{j+2}(tk_xj_new2(i));
    end
end
temp=ones(n,1);
hi_x_new=[temp,hi_x,tk_xj_new'];
hi_x_new2=[temp(1:n-1),hi_x2,tk_xj_new2'];
...
for i=2:iter+1  
    ...
    ua_iter(:,i)=ua_iter(:,i-1) + alpha * delta_a; %(n*1)
    % ua_iter(:,i)=ua_iter(:,i-1)+delta_a;%(n*1)
    u_new=(hi_x_new*ua_iter(:,i));%(n*1)
    u2_new=(hi_x_new2*ua_iter(:,i));
...
% kt1=(u2.^2+u2).*(t(2:length(x))-t(1:length(x)-1))';
% kt2=(u(2:length(x)).^2+u(2:length(x))).*(t(2:length(x))-t(1:length(x)-1))';
% kf=(temp2*kt+4*temp2*kt1+temp2*kt2).*(-1)./6;
% end
%% 问题5 
% function kf=get_kf(x,t,t2,u,u2)
% temp=tril(ones(length(x)-1),-1);
...
temp=tril(ones(length(x)-1),-1);
temp2=[temp;ones(1,length(x)-1)];
% 离散化积分内部包含 t 和 u 的左端点、中点及右端点求值
kt=(u(1:length(x)-1).^2 ./ (1 + t(1:length(x)-1)') ) .* (t(2:length(x))-t(1:length(x)-1))';
kt1=(u2.^2 ./ (1 + t2') ) .* (t(2:length(x))-t(1:length(x)-1))';
kt2=(u(2:length(x)).^2 ./ (1 + t(2:length(x))') ) .* (t(2:length(x))-t(1:length(x)-1))';
% 合并辛普森求积项，并统一乘以 x 相关项：-10 * (1+x) 
kf=(temp2*kt+4*temp2*kt1+temp2*kt2) .* (-10 .* (1 + x')) ./ 6;
end